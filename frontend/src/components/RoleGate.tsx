...
/**
 * The shared on-chain role gate (docs/07 R5, docs/04 §6).
 *
 * Extracted from the Phase-6 issuer gate so the admin panel enforces the SAME
 * state machine rather than a near-copy that could drift — in particular the
 * error-vs-denial distinction below, which is the one a second implementation
 * would most easily get wrong. Callers supply only the authorization predicate
 * and the wording; the states, their order, and the wallet/role plumbing are
 * identical everywhere.
 *
 *   no wallet / not connected → prompt to connect
 ...
 *   connected, unauthorized   → friendly refusal, no dead-ends
 *   connected, authorized     → children (which may assume authorization)
 *
 * Roles come from {@link useRoleRead}, the single role-read path in the app —
...
export interface RoleGateCopy {
  /** Shown when MetaMask isn't installed at all. */
  noProviderTitle: string
  noProviderBody: string
  /** Shown when a wallet exists but isn't connected. */
  connectBody: string
...
interface RoleGateProps {
  copy: RoleGateCopy
  /** True when this role-read result may pass the gate. */
  authorize: (data: RoleReadResult) => boolean
  children: (auth: AuthorizedContext) => ReactNode
}

export function RoleGate({ copy, authorize, children }: RoleGateProps) {
  ...
  // 5. Read succeeded but this wallet doesn't qualify → not authorized.
  if (!authorize(role.data)) {
    return <UnauthorizedGate copy={copy} account={account} />
  }
  ...
  return (
    <Panel>
      ...
      <h2 className="mt-4 font-serif text-xl font-semibold text-brand-900">
        {noProvider ? copy.noProviderTitle : 'Connect your wallet to continue'}
      </h2>
      <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-slate-600">
        {noProvider ? copy.noProviderBody : copy.connectBody}
      </p>
  ...
  return (
    <Panel>
      ...
      <div className="mx-auto mt-5 max-w-md rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-left text-sm">
        ...
        <p className="mt-3 text-slate-600">{copy.unauthorizedHelp}</p>
      </div>
      <div className="mt-6">
        <Link
          to="/"
